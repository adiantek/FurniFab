...
def abort_task(time, tasks, updated_tasks, x, sorted_data, time2, order, index, timeout_seconds):
    ...
    BB(x, copy.deepcopy(sorted_data), time.copy(), time2.copy(), order.copy(), updated_tasks_copy,
       timeout_seconds=timeout_seconds)  #### jednak nie przerywamy zadania
        ...
        if index and n == True:
            if tasks[index] < time[-1] + x[1]:  ### trzeba uwzględnić fakt, że zadanie może zostać przerwane
                abort_task(time.copy(), tasks.copy(), updated_tasks.copy(), x, copy.deepcopy(sorted_data), time2.copy(),
                           order.copy(), index, timeout_seconds=timeout_seconds)
                ...
                do_whole_task(time.copy(), tasks.copy(), updated_tasks.copy(), x, copy.deepcopy(sorted_data),
                              time2.copy(), order.copy(), index, n, timeout_seconds=timeout_seconds)
            ...
            do_whole_task(time.copy(), tasks.copy(), updated_tasks.copy(), x, copy.deepcopy(sorted_data), time2.copy(),
                          order.copy(), index, n, timeout_seconds=timeout_seconds)
...
def run_algorithm(input_data):
    global the_best, time_the_best, time2_the_best, order_the_best, stop_recursion, result, result2
    data = {}
    tasks = []
    the_best = sys.float_info.max
    time_the_best = []
